...
physeq_18S_glom <- tax_glom(physeq_18S_filter1, taxrank=rank_names(physeq_18S_filter1)[7]) 

physeq_16S_filter2 <- microbiome::transform(physeq_16S_glom,'compositional')
physeq_18S_filter2 <- microbiome::transform(physeq_18S_glom,'compositional')

physeq_merge1 <- merge_phyloseq(physeq_16S_filter2,physeq_18S_filter2)

ASV_freq <- as(otu_table(physeq_merge1), "matrix")
...
filter3_BH_pear <- subset(filter2_BH_pear, origin == "Acanthamoeba" | origin == "Echinamoeba"| origin == "Korotnevella"| origin == "Naegleria"| origin == "Tetrahymena"| origin == "Vannella"| origin == "Vermamoeba" | destination == "Acanthamoeba" | destination == "Echinamoeba"| destination == "Korotnevella"| destination == "Naegleria"| destination == "Tetrahymena"| destination == "Vannella"| destination == "Vermamoeba")
filter4_BH_pear <- subset(filter3_BH_pear, value > .7)

pdf("BH_test.pdf", width = 14, height= 14)

...
physeq_merge2 <- microbiome::transform(physeq_16S,'clr')
...
ordcap <- ordinate(physeq_merge2, "CAP", "euclidean", ~type)
pdf("./bdiv_cap.type.16S.pdf")
plot_ordination(physeq_merge2, ordcap, "samples", color="type") + 
    theme_minimal()
dev.off()
permanova_pairwise(otu_table(physeq_merge2), grp=sample_data(physeq_merge2)$type, method="euclidean") # check signficane
ordcap <- ordinate(physeq_merge2, "CAP", "euclidean", ~month)
pdf("./bdiv_cap.month.16S.pdf")
plot_ordination(physeq_merge2, ordcap, "samples", color="month") + 
    theme_minimal()
dev.off()
permanova_pairwise(otu_table(physeq_merge2), grp=sample_data(physeq_merge2)$month, method="euclidean") # check signficane

clr.dist <- dist(otu_table(physeq_merge2), method="euclidean")
#geo locations
dispr <- vegan::betadisper(clr.dist, phyloseq::sample_data(physeq_merge2)$month)
...
bleep <- subset(filter2_BH_pear, origin == "Legionella" | destination == "Legionella") 
bloop <- subset(bleep, value > 0.9)

pdf("bact_july_aug.pdf",width=14, height=14)
chordDiagram(bloop, transparency = 0.5, col = ifelse(bloop$origin == "Legionella", ifelse(bloop$value > 0, ifelse(bloop$value > 0.7, "#0487fb", ifelse(bloop$value > 0.5, "#04F0FB", "#04FB38")), "red") , "gray"),
	annotationTrack = "grid", preAllocateTracks = list(track.height = 0.1), big.gap=0, small.gap=0)
dev.off()
